...
| [2차](docs/rounds/round2.md) | 50 ep, 정지 40% | 배경의 무관한 변화를 동작의 원인으로 잘못 학습 | 확인 | **2/10 (20%)** |
| [3차](docs/rounds/round3.md) | 50 ep, 정지 20% | 정책이 볼 수 없는 정보에 기댄 시연과 프레임 드랍이 정밀 파지를 막는다 | 확인 | **4/20 (20%)** |
...
| [A/B](docs/rounds/experiment-ab.md) | 재학습 없음 | 화면을 더 자주 보고 다시 계산하게 하면 정밀도가 오른다 | **반증** | 0/3, 0/10 |
...
<!--
영상은 이슈/PR 작성창에 드래그해 올린 뒤 나오는
https://github.com/user-attachments/assets/... 주소를 아래 칸에 붙여넣으세요.
파일로 둘 경우 media/round5/ 아래에 넣고 상대경로로 링크합니다. → media/README.md
-->

| | |
|---|---|
| 파지 성공 | *(영상 추가 예정)* |
| 파지 실패 (큐브를 비껴 잡음) | *(영상 추가 예정)* |

**병목이 파지 단계 하나로 좁혀졌습니다.** 컵을 고정하자 파지 이후 구간(이송 → 투입 → 복귀)이 안정화돼 파지 후 실패가 0건입니다. 3차에서는 파지 성공 9건 중 5건이 컵 단계에서 깨졌습니다.

**남은 원인은 큐브 위치를 눈으로 못 짚는 것입니다.** 학습 데이터에서 큐브를 집는 순간의 팔 자세를 뽑아보니 **7개 지점이 서로 겹쳐 있었습니다** — 7지점 전부가 베이스 회전 **12.8°** 안에 몰려 있는데, 같은 지점을 여섯 번 찍은 것끼리도 **±3.3°** 씩 흔들립니다. 옆 지점과 구분이 안 되는 데이터였습니다.

지점별 성공률은 갈렸지만(2·3·4는 성공, 1·5·6은 전멸) **지점당 3회로는 지점마다 난이도가 다른 건지 판단할 수 없습니다.**
