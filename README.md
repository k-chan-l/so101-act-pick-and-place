...
| 3차 | 50 ep (27,147 프레임), 정지 10 (20%) / 증강 on | **4/20 (20%)** — 최고 성능 | 병목이 파지 정밀도(10)와 컵 단계(5)로 갈림 → 복구 시연 보강 |
| 4차 | 3차 + DAgger 교정 17 ep = 67 ep (33,547 프레임) / 3차 가중치에서 40k fine-tune, 증강 off | **0/10 (회귀)** | 교정 데이터가 좁은 관절 영역에 뭉쳐 평균화 → 허공 잡음. 17 ep 보류 |
| 4b | 4차와 동일 조건, 증강만 on | **0/10** (근접 10/10) | 허공 잡음의 주범은 **증강 제거**로 확정. 다만 파지 성공 3차 45% → 20%라 **DAgger 17 ep는 제외** |
| 5차 (촬영 중) | 신규 50 ep (정상 42 = 7지점 × 6회, 정지 8), DAgger 제외, from scratch 100k / 증강 on | — | **정렬 구간을 1~2초 천천히** 기록, 각도 0°/45° 절반씩, 카메라 노출 고정, 파지 후 멈춤 없이 곧장 컵. 평가 7지점 × 3회 = 21회 |

> **1~4차 공통 근본 문제** — 정책은 "큐브가 보이면 잡는다"는 배웠지만 **"큐브를 손목캠 중앙에 정렬한 뒤 잡는다"는 배우지 못했습니다.** 3차·4b의 시연자 관찰과 A/B 실험 결과가 모두 같은 곳을 가리킵니다. 5차 시연 프로토콜의 "정렬 구간 1~2초"가 이에 대한 조치입니다.
